...
function inquirerForUpdates(){
	inquirer.prompt([{
		name:"action",
		type: "list",
		message: "Select an option to manage current inventory:",
...
function restockRequest(){
	inquirer.prompt([
	{
		name:"ID",
		type:"input",
		message:"What is the product number of the product you would like to restock?"
	},
	{
		name:"Quantity",
		type:"input",
...
function restockInventory(id, quant){
	connection.query('SELECT * FROM Products WHERE product_id = '+id, function(err,res){
		if(err){console.log(err)};
		connection.query('UPDATE Products SET stock_quantity = stock_quantity + ' +stock_quantity+ 'WHERE product_id =' + product_id);
...
function addRequest(){
  ...
  function buildNewItem(name,category,price,quantity){
  	connection.query('INSERT INTO products (product_id,product_name,department_name,price,stock_quantity) VALUES("' + id + '","' + name + '","' + category + '",' + price + ',' + quantity +  ')');
  	displayInventory();
  };
